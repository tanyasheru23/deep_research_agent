...
from app_agents.writer_agent import write_report, ReportData
...
class ResearchManager:
                ...
                try:
                    ...
                    async with Session() as session:
                        session.add(Report(
                            ...
                            word_count = len(report.markdown_report.split()),
                            depth      = depth,
                            created_at = time.time(),
                        ))
                        await session.commit()
                    yield "💾 Report saved to history"
                except Exception as e:
                    yield f"⚠️ Could not save report: {e}"
    ...
    #         for r in results
    #     )
    #     original = editor_agent.model
    #     editor_agent.model = model
    #     try:
    #         result = await Runner.run(
    #             editor_agent,
    #             f"Original query: {query}\nRequested depth: {depth}\n\n"
    ...
    #     finally:
    #         editor_agent.model = original
    async def _write_report(self, query, results, depth, model) -> ReportData:
        return await write_report(query, results)
